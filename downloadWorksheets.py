...
g_index_link = r'http://www.kids-pages.com/index.htm'
g_current_root = r'http://www.kids-pages.com/folders/worksheets/'
pdf_links = []
have_visited_links = []
...
def downlaodWorkSheetParseHtml(url):
    if str(url).find(g_site) < 0:
        return
    
    if str(url).find(g_index_link) > 0:
        return

    if str(url).find(g_current_root) < 0:
        return;
    ...
    for urlitem in link_list:
        ...
        if urlitem.find(".pdf") >0 :
            #pdfUrl = url + urlitem
            AppendPdfLinks(urlitem) #pdf_links.append(urlitem);
        else:
            downlaodWorkSheetParseHtml(urlitem) 


def downloadWorkSheet(url):
    if str(url).find(g_site) < 0:
        return
    ...
    for urlitem in link_list:
        ...
        if urlitem.find(r'/') > 0:
            newUrl = url + urlitem
            print newUrl
            downlaodWorkSheetParseHtml(newUrl)
...
if __name__ == '__main__':
    ...
    downloadWorkSheet(r'http://www.kids-pages.com/folders/worksheets/')
    #downloadPdf(g_site)
